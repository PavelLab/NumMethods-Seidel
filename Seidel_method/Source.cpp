...
int main(void)
{
	...
	system("title Родионов. Задача Дирихле для уравнения Пуассона");
	...
	double	a, b, c, d;			//Границы области определения уравнения
	int Exit = 1, Show = 0;
	...
	while (Nmax <= 0)
	{
		cout << endl << "Максимальное количество итераций: ";
		cin >> Nmax;
	}
	system("cls");
	...
	cout << "Полученное численное решение:" << endl;
	ShowSolution(V, n, m);
	cout << endl << "---------------------------------------------" << endl;
	//cout << "Точное решение:" << endl;
	//Exact_solution(n, m, a, b, c, d);

	//Справка
	cout << endl << "---------------------------------------------" << endl;
	cout << "Размерность сетки: (" << n << ", " << m << ")" << endl;
	cout << "Область определения X: [" << a << "; " << b << "] "<< "Область определения Y: [" << c << "; " << d << "]" << endl;
	...
	cout << "Достигнутая точность: " << fixed << setprecision(10) << scientific << epsMax << endl;
	cout << "Погрешность решения: " << fixed << setprecision(10) << scientific << CheckNumSolution(V, n, m, a, b, c, d) << endl;
